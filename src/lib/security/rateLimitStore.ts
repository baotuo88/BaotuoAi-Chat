...
function canUseMemoryFallback(): boolean {
  return isMemoryStoreFallbackAllowed();
}
...
export function createRateLimitStore(): RateLimitStore {
  ...
  // Otherwise hosted mode requires a shared store unless the operator opts into
  // the per-instance memory fallback (single-instance self-hosting).
  if (
