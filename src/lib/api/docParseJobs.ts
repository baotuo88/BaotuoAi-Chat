...
function canUseMemoryFallback(): boolean {
  return getDeploymentMode() === "local" || isMemoryStoreFallbackAllowed();
}
...
function createDocumentParseJobStore(): DocumentParseJobStore {
  ...
  // An explicit shared-store request without credentials is a misconfiguration.
  // Hosted mode also defaults to requiring a shared store, unless the operator
  // opts into the per-instance memory fallback (single-instance self-hosting).
  if (
    isSharedStoreName(store) ||
    (getDeploymentMode() === "hosted" && !isMemoryStoreFallbackAllowed())
  ) {
