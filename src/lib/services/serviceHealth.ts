...
function storeHealth(
  ...
  // Explicitly requesting a shared store without credentials is always a
  // misconfiguration, regardless of the memory-fallback opt-in.
  if (wantsSharedStore) {
    return item(service, "policy_blocked", "SHARED_STORE_REQUIRED");
  }

  // Hosted mode without a shared store: allowed only when the operator opts
  // into the per-instance memory fallback (single-instance self-hosting).
  // Surface it as a warning so multi-instance risks stay visible.
  if (hosted) {
    if (isMemoryStoreFallbackAllowed()) {
      return item(service, "local_only", "MEMORY_STORE_FALLBACK");
    }
    return item(service, "policy_blocked", "SHARED_STORE_REQUIRED");
  }
