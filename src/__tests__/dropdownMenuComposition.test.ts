...
describe("dropdown menu composition", () => {
  ...
  it("wires the about settings tab to a product-info layout", () => {
    ...
    // Brand strings are centralized in config/brand.ts; AboutSettings imports
    // them rather than hardcoding, so assert the wiring plus the source values.
    expect(aboutSettings).toContain("config/brand");
    expect(aboutSettings).toContain("BRAND_REPO_URL");
    expect(aboutSettings).toContain("BRAND_LICENSE_NAME");
    expect(brand).toContain("baotuo88/BaotuoAi-Chat");
    expect(brand).toContain("MIT License");
