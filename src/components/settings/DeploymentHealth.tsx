...
const DeploymentHealth: React.FC = () => {
  ...
  const items: HealthItem[] = [
    ...
    (() => {
      // Resolve a single HealthState first, then derive both the dot color
      // and the descriptive text from it so they can never disagree (e.g.
      // showing a yellow "warning" dot next to text implying it's broken).
    ...
    (() => {
      const hostedWithoutSharedRegistry =
        deploymentMode === "hosted" &&
        deployment?.pluginRegistryStore !== "shared";
      // Same single-source-of-truth pattern as the "stores" card: resolve
      // state once, derive detail from it so color and text stay aligned
      // (e.g. hosted + memory fallback should read as a warning, not blocked).
      const state: HealthState = hostedWithoutSharedRegistry
        ? runtimeState("pluginRegistry") || "blocked"
        : installedPlugins.length > 0
          ? "ok"
          : runtimeState("pluginRegistry") || "missing";
      const detail = hostedWithoutSharedRegistry
        ? state === "warning"
          ? t("storesMemory")
