...
Baotuo Chat is local-first by default. A production self-hosted deployment should
...
spoofed forwarded headers before requests reach Baotuo Chat. These headers affect
...
### Vercel Without Upstash (Single-Instance / Hobby)

A personal or low-traffic Vercel project running in hosted mode may not want
to provision Upstash, and may want to point providers at a custom or relay
OpenAI-compatible endpoint instead of an official upstream. Two opt-in flags
relax those specific defaults while keeping every other hosted-mode
protection (request proof, access control, SSRF/URL policy, CSP) active:

```bash
DEPLOYMENT_MODE=hosted
ALLOW_CUSTOM_PROVIDER_BASE_URL=true
ALLOW_MEMORY_STORE_FALLBACK=true
BYOK_ALLOW_EPHEMERAL_KEY=false
BYOK_PRIVATE_KEY_PEM="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----"
BYOK_KEY_ID=prod-2026-07
```

- `ALLOW_CUSTOM_PROVIDER_BASE_URL=true` lets users configure a custom provider
  base URL in hosted mode. Public HTTPS endpoints only; localhost and private
  networks stay blocked by the existing SSRF checks. Without this flag, hosted
  mode pins providers to their official base URLs and rejects custom ones.
- `ALLOW_MEMORY_STORE_FALLBACK=true` lets rate limiting, document parse jobs,
  and the plugin registry fall back to per-instance in-memory storage when
  `RATE_LIMIT_STORE`/`DOCUMENT_PARSE_JOB_STORE`/`PLUGIN_REGISTRY_STORE` are not
  set to `upstash`. Without this flag, hosted mode without Upstash throws at
  store-creation time — the store dependent features stop working outright.

**Trade-off:** in-memory stores are per-instance and are NOT shared across
Vercel serverless function instances. Rate limiting becomes weaker under
concurrent load, and document parse job polling can miss a job that was
created on a different instance than the one now polling for it. This is
acceptable for a single-instance/Hobby-tier deployment; provision Upstash and
leave both flags unset once the deployment scales to concurrent instances or
meaningful traffic. `BYOK_PRIVATE_KEY_PEM` must still be a stable key set as
an env var — relying on `BYOK_ALLOW_EPHEMERAL_KEY` on a serverless platform
means every cold start generates a different keypair in memory, so
client-encrypted secrets from one instance fail to decrypt on another.
Check the Deployment Health panel (`/api/health`) after configuring these to
confirm every service reports back as available or an expected warning
rather than blocked.
...
not a user account system. Before offering Baotuo Chat as a public multi-user SaaS,
