...
Baotuo Chat is local-first by default. Most user settings can be configured in the
...
| Variable                 | Purpose                                                                                                                      |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `DATABASE_URL`           | Optional. Setting this enables email/password accounts, per-user sessions, and daily request quotas. Uses Neon serverless Postgres over HTTP, so it works in edge/serverless/Workers runtimes. |
| `ACCOUNT_SESSION_SECRET` | Required when `DATABASE_URL` is set. Signs per-user session cookies (HMAC-SHA256). Generate with `openssl rand -base64 32`. |
| `DEFAULT_DAILY_QUOTA`    | Default daily request quota per user when a user's `dailyQuota` column is `null`. Defaults to `200`. Counts requests to metered AI routes (chat, search, RAG, voice, doc-parse), not tokens. |

Accounts are entirely optional and layer on top of `ACCESS_PASSWORD`, not a
replacement for it. Chat history and knowledge bases remain client-side
(IndexedDB/OPFS) regardless of whether accounts are enabled — only account
records, quota counters, and audit logs (`users` / `auditLogs` tables) live in
the database. There is intentionally no admin UI for managing users, quotas,
or bans: disable an account, change a quota, or force-invalidate a user's
sessions (by bumping `tokenVersion`) by editing the database rows directly.

### First-time database setup

The repo ships an initial Drizzle migration at `drizzle/0000_initial_accounts.sql`.
Before the first deploy that sets `DATABASE_URL`, apply the schema against the
target Neon database — either by running the SQL file directly, or with:

```bash
pnpm db:push
```

To evolve the schema later, edit `src/lib/db/schema.ts` and run
`pnpm db:generate` to produce the next numbered migration file. Commit the
generated SQL alongside the schema change so every deployment applies the same
migrations.
...
and rate limiting. Leave it `false` unless Baotuo Chat is behind a trusted proxy
