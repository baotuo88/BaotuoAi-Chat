...
Baotuo Chat is local-first. Durable user data stays in browser storage whenever
...
Baotuo Chat uses several browser storage layers:
...
Baotuo Chat validates request payloads, applies URL safety gates, limits response
...
it does not turn Baotuo Chat into a full public SaaS security boundary.

Before offering Baotuo Chat as a public multi-user service, add:
